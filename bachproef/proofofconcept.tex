...
In deze sectie bespreken we de specificaties van de Android-applicatie om compatibiliteit en consistentie met de eerder genoemde scripts en modellen te waarborgen. Het manifestbestand (\autoref{lst:android-manifest}) bevat de nodige instellingen om de applicatie correct te laten functioneren op mobiele apparaten. Hierin worden activiteiten zoals \texttt{ProcessTextActivity}, de bijhorende \texttt{intents} en \gls{services} zoals \texttt{SpeedTestService} gedefinieerd, wat essentieel is voor de verwerking en functionaliteit van de applicatie. 
Daarnaast zorgt de \texttt{libs.versions.toml} file (\autoref{lst:libs-versions}) voor het beheren van consistente versies van bibliotheken in de gehele applicatie. Deze bevat de versie-informatie voor onze bibliotheken zoals \texttt{Kotlin}, \texttt{TensorFlow}, en \texttt{Compose}.
Om de modellen lokaal te gebruiken, plaatsen we handmatig een van de geconverteerde en gekwantiseerde modellen en hun vocabulaire in de \texttt{assets} map van de Android-applicatie, zoals weergegeven in \ref{fig:androidassets}.
...
\begin{listing}[ht]
    \begin{minted}[fontsize=\small]{kotlin}
        ...
        \end{minted}
        \caption[\texttt{MainActivity.kt} (deel 1)]{Het eerste deel van \texttt{MainActivity.kt} initialiseert het \glsxtrshort{tflite}-model en laadt het vocabulairebestand, met behulp van functies zoals \texttt{loadVocabulary} uit het utility-object \autoref{lst:load-vocabulary}.}
...
De \texttt{ProcessTextActivity} wordt geactiveerd wanneer een gebruiker tekst selecteert en de optie "Inference" kiest, zoals weergegeven in \ref{fig:intent-example}. Dit stelt de gebruiker in staat om tekst uit andere applicaties te verwerken.
...
\begin{listing}
    ...
    \end{minted}
    \caption[\texttt{SpeedTestService.kt}]{De \texttt{SpeedTestService} voert batchverwerking en snelheidstesten uit voor het \glsxtrshort{tflite}-model, vergelijkbaar met de Python-scripts. De getoonde code is ingekort, maar functioneert op dezelfde manier als de activities in het project.}
